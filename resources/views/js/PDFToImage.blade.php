<script>
    ...
    upload.addEventListener("click", function(){
        if(Data != undefined){
            progress.innerHTML = "{{__('PDFToImage.uploading')}}";
            post_file();
        }else{
            hint.innerHTML = "{{__('PDFToImage.hint')}}";
        }  
    });
    ...
    async function post_file(){
        ...
        Type = fileTypes[0];
        const url = "{{url('/api/PDFToImage/convert')}}" + "?type=" + type;   
        progress.innerHTML = "{{__('PDFToImage.converting')}}";
        ...
        if(rawResponse.ok){
            ...
            progress.innerHTML = "{{__('PDFToImage.done')}}";
        }else{
            //format not match
            const response = await rawResponse.json();
...
<script type="module">
    ...
    function play(time){
        ...
        if(progress.innerHTML == "{{__('PDFToImage.uploading')}}" || progress.innerHTML == "{{__('PDFToImage.converting')}}"){
