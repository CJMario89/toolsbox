<script>
    ...
    function convertUnit(value, from_unit, to_unit) {
        ...
        if(regex.test(value) && !regex2.test(value)){
            ...
            }else if(result <= 0.0000001 && result != 0 && result != ""){
                result = result.toExponential().replace(/e\+?/, ' x 10^');
                var e = ' x 10^';
                if(result.indexOf(e) > 7){
                    if(carry != 0){
    ...
    function fillRateId(exchangeRate){
        var i = 0;
        for(let [country, rate] of Object.entries(exchangeRate)){
            var option = document.querySelectorAll("." + CSS.escape(country));
            if(option.length > 0){
                option[0].id = rate + "fr";
                option[1].id = rate + "to";
            }
